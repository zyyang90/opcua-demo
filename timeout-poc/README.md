# timeout-poc — 复现 taosx-opc 在某第三方 OPC UA Server 上的两个报错

## 目标

现场使用某第三方 OPC UA Server 时,`taosx-opc points -c a.toml` 命令出现两个报错(时而其一、时而其二):
...
该第三方 OPC UA Server 上报的是虚高的 **65536**,于是 taosx 会把上万个节点塞进**一次** Read/Browse 请求。这种较慢的 server 无法在 `request_timeout` 内完成这么大的单批请求 → 返回 `StatusBadTimeout`。
...
该第三方 OPC UA Server 的 license 限制了**并发会话数**。当 taosx 因报错②反复重跑、上一次的会话尚未被 server 回收时,会话数撞到 license 上限,新的 Connect 直接被拒。

本服务器用 `--license-max` 限制会话数,超限时在 `CreateSession` 阶段直接返回 `BadLicenseLimitsExceeded`(状态码与现场一致)。
